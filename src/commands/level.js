...
module.exports = {
  ...
  async execute(interaction) {
    // Defer reply because generating the rank card may take time (canvas + DB)
    ...
    try {
      ...
      console.error('Error executing /level command:', err);
      if (interaction.deferred || interaction.replied) {
        await interaction.editReply({ content: 'حدث خطأ أثناء إنشاء بطاقة الرتبة.' });
      } else {
        await interaction.reply({ content: 'حدث خطأ أثناء إنشاء بطاقة الرتبة.', ephemeral: true });
      }
    }
  }
};
